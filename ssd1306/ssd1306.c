...
#define ssd1306_command1(oled, c) (*(((oled)->i2c_fun).command1_fun))(oled, c)
#define ssd1306_command_list(oled, p, n) (*(((oled)->i2c_fun).command_list_fun))(oled, p, n)
#define ssd1306_framebuf_transmit(oled) (*(((oled)->i2c_fun).framebuf_transmit_fun))(oled)
...
void oled_init(ssd1306_oled *oled, void *i2c, uint8_t addr, uint8_t vccstate, uint16_t w, uint16_t h)
{
	...
	oled->i2cdev = i2c;
	oled->i2c_addr = addr;
	oled->frameBuffer = (uint8_t *)malloc((w * h) / 8);
	oled_clear(oled);
